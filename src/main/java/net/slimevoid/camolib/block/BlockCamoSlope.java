package net.slimevoid.camolib.block;
...
import net.slimevoid.camolib.item.SlopeItem;
...
public class BlockCamoSlope extends Block implements ITileEntityProvider {
    ...
    public int shapeCat;
    public BlockCamoSlope(String name,Material materialIn,int shapeCat) {
        super(materialIn);
    ...
    protected BlockStateContainer createBlockState() {
        return new CamoExtendedBlockState(this, new IProperty[] {CAMO,TYPE[this.shapeCat],DIRECTIONQUAD}, ArrayUtils.addAll(new IUnlistedProperty[]{FACING,HFACING,POS},BLOCKSTATES));
    }
    ...
    public int getMetaFromState(IBlockState state) {
        EnumSlope currentSlope = (EnumSlope) state.getValue(TYPE[this.shapeCat]);
        int start = this.shapeCat*16;
        int end = Math.min(start+16,EnumSlope.values().length);
        for (int i =start ; i < end; i++) {
            if (currentSlope == EnumSlope.values()[i]) {
                return i%16;
            }
        }
        return 0;
    ...
    public IBlockState getStateFromMeta(int meta) {
            return this.getDefaultState().withProperty(TYPE[this.shapeCat], EnumSlope.values()[(this.shapeCat*16)+meta]);
    }
    @Override

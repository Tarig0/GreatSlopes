package net.slimevoid.greatSlopes.client.renderer.block.model;
...
public class ModelBlockCamo implements IBakedModel {
    ...
    public List<BakedQuad> getQuads(IBlockState state, EnumFacing facing, long rand) {
        //get model of block if we didn'SlopeFactory camo this will allow multiple models to be handled
    ...
    public TextureAtlasSprite getParticleTexture() {
        return FMLClientHandler.instance().getClient().getBlockRendererDispatcher().getBlockModelShapes().getModelForState(ConfigLib.CamoBlocks.get(0).getDefaultState()).getParticleTexture();
    }
